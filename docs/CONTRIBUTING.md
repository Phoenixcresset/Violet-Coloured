...
Run this script to be able to run shell files via terminal :

```shell
pnpm config set scriptShell "C:\\Program Files\\git\\bin\\bash.exe"
```

BEFORE cloning, if you are on Windows, you must :

- Enable [Developper Mode](https://learn.microsoft.com/en-us/windows/advanced-settings/developer-mode)
- Execute this script :
...
TODO : documentation to adapt to this modpack
...
- `Curseforge slug`: A unique identifier for a project used by Pakku. To get the slug of a mod, look at the URL of its main page. For instance, `https://modrinth.com/mod/sodium` means that Sodium's slug is `sodium`
